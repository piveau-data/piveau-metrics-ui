    ...
    <div class="container">
      <div class="Search__container">
        ...
        <div class="Chart__content">
            <p style="text-align: center">{{ $t("message.dashboard.licence_usage.plots.title.most_used_licence") }}</p>
...
export default {
  ...
  methods: {
    requestData () {
      axios.get(this.$env.ROOT_API + '/metric/global/datasets/licences', {
        auth: {
  ...
  filters: {
    ...
    hope: function (objects, key) {
      return objects.map(function (object) {
        return round(object[key]) // (objectt[key], 2) >> 2 Stellen nachm Komma
      })
    }
  },
  created: function () {
