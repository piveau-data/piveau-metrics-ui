    ...
    <div class="container">
      <div class="Search__container">
        ...
        <div class="Chart__content">
            <p style="text-align: center">{{ $t("message.dashboard.accessibility.plots.title.accessibility_of_distributions") }}</p>
            <div v-if="loaded == true" class="relative">
            <div v-if="!data.success || data.success == false" class="relative">
              <p class="text-muted" style="text-align: center" id="no-data">  {{ $t("message.no_data") }} </p>
            </div>
...
export default {
  ...
  methods: {
    requestData () {
      axios.get(this.$env.ROOT_API + '/metric/catalogues/' + this.id + '/distributions/accessibility', {
        auth: {
